...
def load_LLM():
    ...
    # Pass press directly in the pipeline as press=press
    pipe = pipeline(
        "text-generation",
        model=model,
        tokenizer=tokenizer,
        device=0,  # Use CUDA device 0
        framework='pt',  # Specify the framework as PyTorch
        press=press  # Add press directly as a pipeline argument
    )
...
def ask_LLM(system_prompt, text, generation_args):
    ...
    # Using the pipeline with `press` functionality
...
col_name = 'context'  # add the column name that contains the text  
...
i = 0
for input_text in concatenated_texts[0:rows]:  # change row to rows
    i = i + 1
    print(i)
